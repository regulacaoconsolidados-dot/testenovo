...
function renderPercentReferenceTable(tbodyId, valueMap, referenceMap, color = "#0b5e42") {
  ...
  tbody.innerHTML = arr.map(([name, value]) => { 
    ...
    const showTextInside = barWidth > 20;
    
    const barHtml = `<div class="progress-bar-container" style="position: relative;">
                       <div class="progress-bar-fill" style="width: ${barWidth}%; background: linear-gradient(90deg, ${color}, ${color}dd);">
                         ${showTextInside ? `<span style="color: white; font-weight: bold; font-size: 11px;">${displayPercent}</span>` : ''}
                       </div>
                       ${!showTextInside ? `<span style="position: absolute; left: 100%; margin-left: 8px; color: #333; font-weight: bold; font-size: 11px;">${displayPercent}</span>` : ''}
                     </div>`;
    
    return `<tr><td title="${escapeHtml(name)}">${escapeHtml(truncateLabel(name, 45))}</td><td class="text-right font-700">${value.toLocaleString("pt-BR")}</td><td style="width:40%;">${barHtml}</td></tr>`; 
  }).join("");
}
...
function renderPercentageTotalTable(tbodyId, dataMap, color = "#0b5e42") {
  ...
  tbody.innerHTML = arr.map(([name, value]) => { 
    ...
    return `<tr><td title="${escapeHtml(name)}">${escapeHtml(truncateLabel(name, 55))}</td><td class="text-right font-700">${value.toLocaleString("pt-BR")}</td><td style="width:40%;">${barHtml}</td></tr>`; 
  }).join("");
}
...
function renderVisaoGeral(filteredFila, filteredAgVivver, filteredAgendados, filteredFaturado, filteredFinanceiro) {
  // CORREÇÃO: Usar os dados já filtrados (filteredFila) para o gráfico
  console.log("Dados da fila após filtros:", filteredFila);
...
function renderEstabelecimento(filteredAgVivver, filteredAgendados, filteredFaturado, filteredFinanceiro) {
  ...
  // CORREÇÃO: Oferta = REC + FAL (soma de recepcionados e faltosos por estabelecimento)
  ...
  // Aumentando o tamanho da fonte nos datalabels e nos ticks dos eixos
  makeHorizontalBarChart("cAgendadasPorEstab", topAgendados.map(([k]) => truncateLabel(k, 28)), topAgendados.map(([,v]) => v), "#b6923e", "Agendadas", false, 14);
  makeHorizontalBarChart("cOfertasPorEstab", topOfertas.map(([k]) => truncateLabel(k, 28)), topOfertas.map(([,v]) => v), "#d97706", "Ofertas", false, 14);
  makeHorizontalBarChart("cRecepcionadosPorEstab", topRecep.map(([k]) => truncateLabel(k, 28)), topRecep.map(([,v]) => v), "#059669", "Recepcionados", false, 14);
  makeHorizontalBarChart("cFaltososPorEstab", topFalt.map(([k]) => truncateLabel(k, 28)), topFalt.map(([,v]) => v), "#dc2626", "Faltosos", false, 14);
  makeHorizontalBarChart("cFaturadosPorEstab", topFatQtd.map(([k]) => truncateLabel(k, 28)), topFatQtd.map(([,v]) => v), "#2563eb", "Faturados", false, 14);
  makeHorizontalBarChart("cFinanceiroPorEstab", topFinanceiro.map(([k]) => truncateLabel(k, 28)), topFinanceiro.map(([,v]) => v), "#059669", "Financeiro", true, 14);
}
