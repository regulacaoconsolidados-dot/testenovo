...
// ============================================================
// FUNÇÃO CORRIGIDA - VERSÃO SIMPLIFICADA E ROBUSTA
// ============================================================
async function loadCSVSmart(url, sheetName) {
  console.log(`Carregando: ${sheetName} - ${url}`);
  ...
  try {
    ...
    console.log(`Texto recebido (${sheetName}): ${text.length} caracteres, primeiros 200:`, text.substring(0, 200));
    
    const parsed = Papa.parse(text, { skipEmptyLines: false, header: false });
    const rawRows = parsed.data || [];
    console.log(`Linhas brutas (${sheetName}): ${rawRows.length}`);
    
    if (rawRows.length < 2) {
      console.warn(`${sheetName}: Poucas linhas encontradas`);
      return [];
    }
    
    // PULA LINHA 1 (vazia ou título) - USA LINHA 2 COMO CABEÇALHO
    const headers = (rawRows[1] || []).map(h => normalizeText(h));
    console.log(`Cabeçalhos (${sheetName}):`, headers);
    
    // DADOS começam na LINHA 3
    const dataRows = rawRows.slice(2);
    console.log(`Linhas de dados (${sheetName}): ${dataRows.length}`);
    ...
    console.log(`Registros processados (${sheetName}): ${result.length}`);
    if (result.length > 0) {
      console.log(`Primeiro registro (${sheetName}):`, result[0]);
    }
    
    return result;
  } catch (e) {
    console.error(`Erro ao carregar ${sheetName}:`, e);
    toast(`Erro ao carregar ${sheetName}: ${e.message}`, "error");
    return [];
  }
}
...
async function loadAllData() {
  ...
  try {
    ...
    const [filaRaw, agVivverRaw, faturadoRaw, financeiroRaw, agendadosRaw] = await Promise.all([
      loadCSVSmart(URL_FILA, "Fila de Espera"),
      loadCSVSmart(URL_AGENDAMENTOS_VIVVER, "Agendamentos Vivver"),
      loadCSVSmart(URL_FATURADO, "Faturado"),
      loadCSVSmart(URL_FINANCEIRO, "Financeiro"),
      loadCSVSmart(URL_AGENDADOS, "Agendados")
    ]);

    console.log("=== RESUMO DO CARREGAMENTO ===");
    console.log(`Fila: ${filaRaw.length} registros`);
    console.log(`Vivver: ${agVivverRaw.length} registros`);
    console.log(`Faturado: ${faturadoRaw.length} registros`);
    console.log(`Financeiro: ${financeiroRaw.length} registros`);
    console.log(`Agendados: ${agendadosRaw.length} registros`);

    // Extrai períodos para determinar o ano atual
    const candidatePeriods = agVivverRaw.map(r => getField(r, ["MÊS"]));
    currentYearShort = getDominantYearShort(candidatePeriods);
    console.log(`Ano atual identificado: 20${currentYearShort}`);
    ...
    console.log(`Fila processada: ${dadosFila.length} registros`);
    ...
    // ========================================
    // PROCESSAMENTO AGENDAMENTOS VIVVER
    // ========================================
    ...
    console.log(`Vivver processado: ${dadosAgendamentosVivver.length} registros`);

    // ========================================
    // PROCESSAMENTO FATURADO (Quantidade)
    // ========================================
    dadosFaturado = [];
    ...
    console.log(`Faturado processado: ${dadosFaturado.length} registros`);

    // ========================================
    // PROCESSAMENTO FINANCEIRO (Valores R$)
    // ========================================
    dadosFinanceiro = [];
    ...
    console.log(`Financeiro processado: ${dadosFinanceiro.length} registros`);

    // ========================================
    // PROCESSAMENTO AGENDADOS
    // ========================================
    dadosAgendados = [];
    ...
    console.log(`Agendados processado: ${dadosAgendados.length} registros`);
    ...
    console.log(`Total de especialidades: ${especialidadesSet.size}`);
    console.log(`Especialidades:`, [...especialidadesSet].slice(0, 20));
    console.log(`Total de grupos: ${gruposSet.size}`);
    
    if (dadosFila.length === 0 && dadosAgendamentosVivver.length === 0 && dadosFaturado.length === 0) {
      toast("ATENÇÃO: Nenhum dado foi carregado! Verifique as URLs das planilhas.", "error");
    } else {
      toast(`Dados carregados: ${dadosFila.length} fila, ${dadosAgendamentosVivver.length} vivver, ${dadosFaturado.length} faturado, ${dadosFinanceiro.length} financeiro`, "success");
    }
    ...
    toast(`Erro fatal: ${e.message}`, "error");
  } finally {
...
function populateFilters() {
  ...
  if (grupoSelect) {
    grupoSelect.innerHTML = '<option value="">Todos os grupos</option>' + 
      [...gruposSet].sort().map(cod => `<option value="${escapeHtml(cod)}">${escapeHtml(GRUPOS_SIGTAP[cod] || cod)}</option>`).join("");
  }
  
  const periodoSelect = el("periodoSelect");
  if (periodoSelect) {
    periodoSelect.innerHTML = '<option value="">Todos os meses</option>' + 
  ...
  if (tableMonthFilterFisico) {
    tableMonthFilterFisico.innerHTML = '<option value="">Todos os meses</option>' + 
      allPeriodos.map(p => `<option value="${escapeHtml(p)}">${escapeHtml(p)}</option>`).join("");
  }
  
  const tabelaMonthFilterEspec = el("tabelaMonthFilterEspec");
  if (tabelaMonthFilterEspec) {
    tabelaMonthFilterEspec.innerHTML = '<option value="">Todos os meses</option>' + 
      allPeriodos.map(p => `<option value="${escapeHtml(p)}">${escapeHtml(p)}</option>`).join("");
  }
...
function buildSubgrupoList() {
  ...
  list.innerHTML = arr.length ? 
    arr.map(sub => `<div class="ms-item" data-value="${escapeHtml(sub)}"><input type="checkbox" value="${escapeHtml(sub)}" ${selectedSubgrupos.has(sub) ? "checked" : ""}><span>${escapeHtml(sub)}</span></div>`).join("") : 
...
function buildEspecialidadeList() {
  ...
  list.innerHTML = arr.length ? 
    arr.map(item => `<div class="ms-item" data-value="${escapeHtml(item)}"><input type="checkbox" value="${escapeHtml(item)}" ${selectedEspecialidades.has(item) ? "checked" : ""}><span>${escapeHtml(item)}</span></div>`).join("") : 
...
function updateMsLabelSub() { 
  const label = el("msLabelSub"); 
  if (label) label.textContent = selectedSubgrupos.size ? `${selectedSubgrupos.size} selecionado(s)` : "Todos"; 
}

function updateMsLabelEsp() { 
  const label = el("msLabelEsp"); 
  if (label) label.textContent = selectedEspecialidades.size ? `${selectedEspecialidades.size} selecionado(s)` : "Todos"; 
}

function closeAllDropdowns() { 
  ["Sub", "Esp"].forEach(suf => { 
    const dd = el(`msDropdown${suf}`); 
    const tr = el(`msTrigger${suf}`); 
...
function createGaugeChart(canvasId, percent, color) {
  ...
  charts[canvasId] = new Chart(ctx, {
    ...
    data: { 
      datasets: [
        { data: [50, 30, 20], backgroundColor: ["rgba(220,38,38,.45)", "rgba(217,119,6,.45)", "rgba(5,150,105,.45)"], borderWidth: 0, hoverOffset: 0, weight: 1 },
        { data: [pctVal, 100 - pctVal], backgroundColor: [color, "rgba(226,232,240,0.0)"], borderWidth: 0, borderRadius: 8, hoverOffset: 0, weight: 2 }
      ] 
    },
    options: { 
      rotation: -90, 
...
function applyFilters() {
  ...
  console.log("Totais:", { totalFila, totalRecepcionados, totalFaltosos, totalAgendados, totalFaturadosQtd, totalFinanceiro });
  ...
  const subAgendados = el("subAgendados"); 
  if (subAgendados) subAgendados.textContent = `Base agendados | ${totalAgendados.toLocaleString("pt-BR")} procedimentos`;
  const subRecepcionados = el("subRecepcionados"); 
  if (subRecepcionados) subRecepcionados.textContent = `${agVivverTotal > 0 ? ((totalRecepcionados / agVivverTotal) * 100).toFixed(1) : "0.0"}% da oferta convertida em recepção`;
  const subFaltosos = el("subFaltosos"); 
  if (subFaltosos) subFaltosos.textContent = `${taxaAbsenteismo.toFixed(1)}% sobre a oferta do Vivver`;
  const subFaturadosQtd = el("subFaturadosQtd"); 
  if (subFaturadosQtd) subFaturadosQtd.textContent = `${taxaFaturamento.toFixed(1)}% dos agendados convertidos`;
  const subFinanceiro = el("subFinanceiro"); 
  if (subFinanceiro) subFinanceiro.textContent = `Base financeira consolidada no escopo de período`;
  ...
  const gaugeFatPct = el("gaugeFatPct"); 
  if (gaugeFatPct) { 
    gaugeFatPct.textContent = taxaFaturamento.toFixed(1) + "%"; 
    gaugeFatPct.style.color = fatColor; 
  }
  ...
  const gaugeAbsPct = el("gaugeAbsPct"); 
  if (gaugeAbsPct) { 
    gaugeAbsPct.textContent = taxaAbsenteismo.toFixed(1) + "%"; 
    gaugeAbsPct.style.color = absColor; 
  }
...
function makeHorizontalBarChart(id, labels, values, color, datasetLabel, isMoney = false, dataLabelFontSize = 11) {
  ...
  charts[id] = new Chart(canvas.getContext("2d"), {
    ...
    options: { 
      ...
      plugins: { 
        legend: { display: true, position: "top", labels: { font: { weight: "bold", size: 12 } } }, 
  ...
  charts[id] = new Chart(canvas.getContext("2d"), {
    ...
    options: { 
      ...
      plugins: { 
        legend: { 
          position: "top", 
          labels: { font: { weight: "bold", size: 12 }, usePointStyle: true, pointStyle: "circle", boxWidth: 10 } 
        }, 
        tooltip: { 
          callbacks: { 
            label: ctx => { 
              const val = ctx.raw || 0; 
              return yMoney ? `${ctx.dataset.label}: ${formatMoney(val)}` : `${ctx.dataset.label}: ${val.toLocaleString("pt-BR")}`; 
            } 
          } 
        }, 
        datalabels: { 
    ...
    options: { 
      ...
      plugins: { 
        ...
        datalabels: { 
          ...
          font: { weight: "bold", size: 10 }, 
          formatter: value => { if (!value) return ""; return value.toLocaleString("pt-BR"); }, 
      ...
      scales: { 
        ...
        y: { 
          ...
          grace: "10%", 
          grid: { display: false }, 
          ticks: { font: { weight: "bold" }, callback: value => value.toLocaleString("pt-BR") }, 
          title: { display: true, text: "Oferta / Recepcionados / Faltosos", font: { weight: "bold" } } 
        }, 
        y1: { 
          beginAtZero: true, 
          position: "right", 
          ...
          ticks: { font: { weight: "bold" }, callback: value => value.toLocaleString("pt-BR") }, 
          title: { display: true, text: "Fila de Espera", font: { weight: "bold" } } 
        } 
      } 
    }
  });
...
function renderAgendadasPorEspecialidadeEstabTable(filteredAgendados) {
  ...
  filteredAgendados.forEach(d => { 
    ...
    if (!map.has(key)) map.set(key, { 
      ...
      mes: d.mes, 
      valores: { "Belo Horizonte": 0, "Centro Materno Infantil": 0, "Hospital Municipal de Contagem": 0, "Hospital São José": 0, "Hospital Santa Rita": 0 }, 
      total: 0 
    }); 
  ...
  charts.cAgendadosVsFaturadosMes = new Chart(canvas.getContext("2d"), { 
    ...
    options: { 
      ...
      scales: { 
        ...
        y: { beginAtZero: true, grace: "10%", grid: { color: "rgba(148,163,184,0.10)" }, ticks: { font: { weight: "bold" } } } 
      } 
    } 
  });
}
...
function renderVisaoGeral(filteredFila, filteredAgVivver, filteredAgendados, filteredFaturado, filteredFinanceiro) {
  ...
  if (legendContainer) { 
    legendContainer.innerHTML = `
      <div class="legend-item"><div class="legend-color agendados"></div><span>Agendados</span><span style="font-weight:900;color:var(--primary-dark)">(Média: ${Math.round(agendadosMedia).toLocaleString("pt-BR")})</span></div>
      <div class="legend-item"><div class="legend-color faturados"></div><span>Faturados</span><span style="font-weight:900;color:var(--primary-dark)">(Média: ${Math.round(faturadosMedia).toLocaleString("pt-BR")})</span></div>
    `; 
  ...
  makeLineChart("cReceitaFinanceiraMes", periods, [{ 
...
function renderFinanceiro(filteredFinanceiro) {
  ...
  makeLineChart("cFaturamentoMensal", periods, [{ 
    label: "Financeiro", 
    data: periods.map(p => financeiroPorMes.get(p) || 0), 
    borderColor: "#059669", 
    backgroundColor: "rgba(5,150,105,0.10)", 
...
function renderFinTable(filteredFinanceiro) {
  ...
  const rows = [...map.entries()].map(([estab, vals]) => ({ 
...
function renderFisicoFinanceiro(filteredAgendados, filteredFaturado, filteredFinanceiro) {
  ...
  const addRow = (estabelecimento, mes) => { 
    ...
    if (!map.has(key)) map.set(key, { estabelecimento, mes, agendados: 0, faturadosQtd: 0, financeiroValor: 0 }); 
    return map.get(key); 
  };
...
function renderTableBodyFisico() {
  ...
  if (!rows.length) { 
    tbody.innerHTML = '<tr><td colspan="5">Nenhum dado disponível
let currentTableMonthFilterFisico = "";
let currentChartFilter = null;
...
async function loadCSVSmart(url, sheetName) {
  ...
  try {
    ...
    const result = dataRows.map(row => {
      const obj = {};
      headers.forEach((header, colIdx) => { if (header) obj[header] = row[colIdx] || ""; });
      return obj;
    }).filter(obj => Object.values(obj).some(v => normalizeText(v) !== ""));
    console.log(`${sheetName}: ${result.length} registros`);
    return result;
  } catch (e) {
    console.error(`Erro em ${sheetName}:`, e);
    return [];
  }
}
...
async function loadAllData() {
  ...
  try {
    ...
    const [filaRaw, agVivverRaw, faturadoRaw, financeiroRaw, agendadosRaw] = await Promise.all([
      loadCSVSmart(URL_FILA, "Fila"),
      loadCSVSmart(URL_AGENDAMENTOS_VIVVER, "Vivver"),
      loadCSVSmart(URL_FATURADO, "Faturado"),
      loadCSVSmart(URL_FINANCEIRO, "Financeiro"),
      loadCSVSmart(URL_AGENDADOS, "Agendados")
    ]);
    ...
    especialidadeToGrupos.clear();
    especialidadeToSubgrupos.clear();
    ...
    dadosFila = filaRaw.map(r => {
      ...
      const subgrupoRaw = normalizeText(getField(r, ["Subgrupo", "SUBGRUPO"]));
      const grupoCodigo = extractGrupoCodigo(grupoRaw);
      const dataCorteRaw = normalizeText(getField(r, ["Data Corte/ Fila de Espera"]));
      const dataCorteNorm = normalizePeriodo(dataCorteRaw);
      if (especialidade) especialidadesSet.add(especialidade);
      if (grupoCodigo) gruposSet.add(grupoCodigo);
      addMapSet(especialidadeToGrupos, especialidade, grupoCodigo);
      addMapSet(especialidadeToSubgrupos, especialidade, subgrupoRaw);
      if (dataCorteNorm && periodoSortValue(dataCorteNorm) > latestDateSortValue) {
        latestDateSortValue = periodoSortValue(dataCorteNorm);
        latestDateValue = dataCorteNorm;
      }
      return {
        codigo: normalizeText(getField(r, ["Código do Procedimento"])),
        ...
        grupoCodigo,
        subgrupo: subgrupoRaw,
        complexidade: normalizeText(getField(r, ["Complexidade- Sigtap"])),
        fila: parseNumberBR(getField(r, ["TOTAL"])),
        dataCorte: dataCorteNorm
      };
    }).filter(d => d.especialidade || d.descricao);
    ...
    // Processa VIVVER
    dadosAgendamentosVivver = agVivverRaw.map(r => {
      const especialidade = normalizeText(getField(r, ["ESPECIALIDADE"]));
      const estabelecimento = normalizeText(getField(r, ["ESTABELECIMENTO"]));
      const grupoRaw = normalizeText(getField(r, ["GRUPO"]));
      const subgrupoRaw = normalizeText(getField(r, ["SUBGRUPO"]));
      const grupoCodigo = extractGrupoCodigo(grupoRaw);
      let recepcionados = parseNumberBR(getField(r, ["REC"]));
    ...
    // Processa FATURADO (quantidade)
    dadosFaturado = [];
    faturadoRaw.forEach(row => {
      const especialidade = normalizeText(getField(row, ["Especialidade Descrição"]));
      const estabelecimento = normalizeText(getField(row, ["ESTABELECIMENTO"]));
      const grupoRaw = normalizeText(getField(row, ["GRUPO"]));
      const subgrupoRaw = normalizeText(getField(row, ["SUB GRUPO"]));
      const grupoCodigo = extractGrupoCodigo(grupoRaw);
      ...
      for (let i = 0; i < MONTHS_ORDER.length; i++) {
        ...
        const valor = parseNumberBR(row[`${MONTHS_ORDER[i]}/${currentYearShort}`]);
        if (valor > 0) {
          dadosFaturado.push({ especialidade, estabelecimento, grupo: grupoRaw || (GRUPOS_SIGTAP[grupoCodigo] || ""), grupoCodigo, subgrupo: subgrupoRaw, mes: mesAno, faturadoQtd: valor });
        }
      }
    });

    // Processa FINANCEIRO (valores R$)
    dadosFinanceiro = [];
    financeiroRaw.forEach(row => {
      const especialidade = normalizeText(getField(row, ["ESPECIALIDADE"]));
      const estabelecimento = normalizeText(getField(row, ["ESTABELECIMENTO"]));
      ...
      for (let i = 0; i < MONTHS_ORDER.length; i++) {
        ...
        const valor = parseNumberBR(row[`${MONTHS_ORDER[i]}/${currentYearShort}`]);
        if (valor > 0) {
          dadosFinanceiro.push({ especialidade, estabelecimento, grupo: grupoRaw || (GRUPOS_SIGTAP[grupoCodigo] || ""), grupoCodigo, subgrupo: subgrupoRaw, mes: mesAno, financeiroValor: valor });
        }
      }
    });
    ...
    allPeriodos = sortPeriodos([...dadosFila.map(d => d.dataCorte), ...dadosAgendamentosVivver.map(d => d.mes), ...dadosFaturado.map(d => d.mes), ...dadosFinanceiro.map(d => d.mes), ...dadosAgendados.map(d => d.mes)]);

    console.log(`Dados: Fila=${dadosFila.length}, Vivver=${dadosAgendamentosVivver.length}, Faturado=${dadosFaturado.length}, Financeiro=${dadosFinanceiro.length}, Agendados=${dadosAgendados.length}`);
  ...
  if (cTendenciaFila) {
    charts.cTendenciaFila = new Chart(cTendenciaFila.getContext("2d"), {
      ...
      data: { 
        labels: trendLabels, 
        datasets: [
          { label: "Fila Real", data: [...trendDataReal, ...Array(3).fill(null)], borderColor: "#dc2626", backgroundColor: "rgba(220,38,38,0.10)", borderWidth: 3, fill: true, tension: 0.3, pointBackgroundColor: "#ffffff", pointBorderColor: "#dc2626", pointBorderWidth: 2.5, pointRadius: 5, pointHoverRadius: 8 },
          { label: "Projeção", data: projectionData, borderColor: "#d97706", borderDash: [6, 4], borderWidth: 3, backgroundColor: "rgba(217,119,6,0.05)", fill: false, tension: 0.3, pointBackgroundColor: "#ffffff", pointBorderColor: "#d97706", pointBorderWidth: 2.5, pointRadius: 5, pointHoverRadius: 8 }
        ] 
      },
      options: { 
        responsive: true, maintainAspectRatio: false, layout: { padding: { top: 24, right: 20, bottom: 10, left: 10 } }, 
        plugins: { 
          legend: { position: "top", labels: { font: { weight: "bold" } } }, 
          tooltip: { callbacks: { label: ctx => `${ctx.dataset.label}: ${(ctx.raw || 0).toLocaleString("pt-BR")}` } }, 
          datalabels: { display: true, color: ctx => ctx.dataset.borderColor || "#1F2937", font: { weight: "bold", size: 10 }, formatter: value => value ? value.toLocaleString("pt-BR") : "", align: "top", anchor: "end", offset: 6, clamp: true } 
        }, 
        scales: { 
          x: { grid: { display: false }, ticks: { font: { weight: "bold" } } }, 
          y: { beginAtZero: true, grace: "10%", grid: { color: "rgba(148,163,184,0.12)" }, ticks: { font: { weight: "bold" } } } 
        } 
      }
    });
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const msDropdownSub = el("msDropdownSub"); 
  if (msDropdownSub) msDropdownSub.addEventListener("click", e => e.stopPropagation());
  
  const msDropdownEsp = el("msDropdownEsp"); 
  if (msDropdownEsp) msDropdownEsp.addEventListener("click", e => e.stopPropagation());
});
