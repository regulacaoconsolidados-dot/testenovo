...
// CORREÇÃO: Função melhorada para mostrar porcentagem mesmo em valores pequenos
function renderPercentReferenceTable(tbodyId, valueMap, referenceMap, color = "#0b5e42") {
  ...
  tbody.innerHTML = arr.map(([name, value]) => { 
    ...
    // Garantir que a porcentagem seja sempre visível
    const showPercentInside = percent >= 15; // Mostra dentro da barra se for >= 15%
    ...
    return `<tr>
      ...
      <td style="min-width: 120px; position: relative;">
        <div class="progress-bar-container" style="position: relative; background: #e5e7eb; border-radius: 20px; overflow: hidden; height: 32px;">
          <div class="progress-bar-fill" style="width: ${Math.max(barWidth, 4)}%; background: linear-gradient(90deg, ${color}, ${color}dd); height: 100%; display: flex; align-items: center; justify-content: flex-end; padding-right: 8px; color: white; font-size: 11px; font-weight: 800;">
            ${showPercentInside ? `<span style="position: relative; z-index: 2;">${percentText}</span>` : ""}
          </div>
          ${!showPercentInside && percent > 0 ? `<span style="position: absolute; left: ${Math.min(Math.max(barWidth, 4) + 5, 95)}%; top: 50%; transform: translateY(-50%); font-size: 10px; font-weight: 900; color: #333; white-space: nowrap; background: white; padding: 2px 6px; border-radius: 12px; border: 1px solid ${color};">${percentText}</span>` : ""}
          ${percent === 0 ? `<span style="position: absolute; left: 8px; top: 50%; transform: translateY(-50%); font-size: 11px; font-weight: 800; color: #666;">0%</span>` : ""}
        </div>
      </td>
    </tr>`;
  }).join("");
...
function renderPercentageTotalTable(tbodyId, dataMap, color = "#0b5e42") {
  ...
  tbody.innerHTML = arr.map(([name, value]) => { 
    ...
    const barWidth = maxValue > 0 ? (value / maxValue) * 100 : 0;
    const showPercentInside = percent >= 15;
    const percentText = percent > 0 ? `${percent.toFixed(1)}%` : "0%";
...
// CORREÇÃO PRINCIPAL: Função renderVisaoGeral com dados da fila interagindo com filtros
function renderVisaoGeral(filteredFila, filteredAgVivver, filteredAgendados, filteredFaturado, filteredFinanceiro) {
  // CORREÇÃO 1: Usar os dados da fila que já foram filtrados pelos filtros aplicados
  // CORREÇÃO 2: Incorporar dados retroativos apenas quando NÃO há filtros aplicados
  
  let filaDataToUse = [...filteredFila];
  
  // Verificar se há filtros ativos
  const hasActiveFilters = selectedSubgrupos.size > 0 || selectedEspecialidades.size > 0 || 
                          (el("grupoSelect")?.value || "") !== "" || 
                          (el("periodoSelect")?.value || "") !== "";
  
  console.log("Filtros ativos:", hasActiveFilters);
  console.log("Filtros específicos - Subgrupos:", selectedSubgrupos.size, "Especialidades:", selectedEspecialidades.size);
  
  // Se NÃO houver filtros ativos, incluir os dados retroativos
  if (!hasActiveFilters) {
    console.log("Nenhum filtro ativo - incluindo dados retroativos na fila");
    // Combinar dados da fila principal (já filtrados) com os dados retroativos
    // Nota: dadosFilaRetroativa são todos os dados, sem filtros (pois são históricos)
    filaDataToUse = [...filteredFila, ...dadosFilaRetroativa];
  } else {
    console.log("Filtros ativos - usando apenas dados filtrados da fila principal");
  }
  
  // Agrupar dados da fila por período
  const filaPorMes = aggregateBy(filaDataToUse, d => d.dataCorte, d => d.fila);
  
  // Dados do Vivver (já filtrados)
  ...
  const allPeriodsFromData = getPeriodsFromFilteredData(filaDataToUse, filteredAgVivver, filteredAgendados, filteredFaturado, filteredFinanceiro);
  const periods = allPeriodsFromData.length ? allPeriodsFromData : [];
  
  console.log("Períodos para visão geral:", periods);
  console.log("Dados de fila por mês (filaDataToUse):", filaPorMes);
