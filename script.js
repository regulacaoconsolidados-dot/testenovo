...
// ============================================================
const OPERADORES = {
  '336': 'Giovanna Borello', '531': 'Rosangela de Jesus', '594': 'Ariana Trindade',
  '536': 'Magaly Mendes', '591': 'Ana P Araujo', '535': 'Cristina Fonseca',
  '534': 'Erica Lucia', '533': 'Natalia Bretas', '595': 'Eliane Pereira',
  '540': 'Carolina de Avelar', '541': 'Daniela Fonseca', '532': 'Graziele Alves'
};
...
function norm(str) {
  if (!str) return '';
  return str.toString().toUpperCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/\s+/g, ' ').trim();
}
...
function titleCase(str) {
  ...
  return str.toString().toLowerCase().split(' ').map((w, i) => {
    if (i === 0 || !artigos.includes(w.toUpperCase())) return w.charAt(0).toUpperCase() + w.slice(1);
    return w;
  }).join(' ');
}
...
function formatCBO(nomeCBO, especialidade) {
  if (!nomeCBO) return '';
  let name = nomeCBO.toString().trim().replace(/^M[eé]dico[:\-\s]*/i, '').trim();
  name = titleCase(name);
  if (especialidade) {
    const esp = especialidade.toString().trim();
    const addons = ['Adulto','Infantil','Geral','Pediátrico','Pediátrica'];
    for (const addon of addons) {
      if (norm(esp).includes(norm(addon)) && !norm(name).includes(norm(addon))) { name += ' ' + addon; break; }
    }
  }
  return name;
}
...
function getOperador(codigo) {
  return OPERADORES[(codigo || '').toString().trim()] || 'Outros';
}
...
function toggleMultiSelect(id) {
  ...
  Object.keys(multiSelectState).forEach(key => {
    if (key !== id) { multiSelectState[key].open = false; const w = document.getElementById(key); if (w) w.classList.remove('open'); }
  });
...
function toggleMultiSelectOption(id, value, checked) {
  const state = multiSelectState[id];
  if (!state) return;
  if (checked) { if (!state.selected.includes(value)) state.selected.push(value); }
  else { state.selected = state.selected.filter(v => v !== value); }
  renderMultiSelect(id);
  applyFilters();
}
...
function filterMultiSelectOptions(id, query) {
  const state = multiSelectState[id];
  if (!state) return;
  state.values = !query ? [...state._allValues] : state._allValues.filter(v => v.toLowerCase().includes(query.toLowerCase()));
  renderMultiSelect(id);
}
...
function initTabs() {
  document.querySelectorAll('.tab-btn').forEach(btn => {
    btn.addEventListener('click', function() {
      ...
      if (panel) { panel.classList.add('active'); renderChartsForTab(tabId); }
    });
  });
}
...
function renderChartsForTab(tabId) {
  switch(tabId) {
    case 'visao-geral': renderChartDistrito(); renderChartTipoAtendimento(); renderChartEspecialidade(); renderChartPrestador(); renderChartSituacao(); renderChartMeses(); break;
    case 'tabela': renderTable(); break;
    case 'agendamentos-distrito': renderChartPrimeiraConsultaDistrito(); renderChartRetornoDistrito(); renderChartComparativoDistrito(); renderChartDistritoRosca(); break;
    case 'absenteismo': renderChartAbsenteismoEsp(); renderChartAbsenteismoDist(); renderChartAbsenteismoMensal(); renderChartAbsenteismoPrestador(); break;
    case 'recepcionados': renderChartRecepcionadosDistrito(); renderChartRecepcionadosEspecialidade(); renderChartRecepcionadosPrestador(); renderChartRecepcionadosMensal(); break;
    case 'cancelados': renderChartCancelamentosDist(); renderChartCancelamentosEsp(); renderChartCancelamentosPrestador(); renderChartCancelamentosMensal(); break;
    case 'transferidos': renderChartTransferidosDistrito(); renderChartTransferidosEspecialidade(); renderChartTransferidosPrestador(); renderChartTransferidosMensal(); break;
  }
}
...
function normalizeData(rows) {
  return rows.map(row => {
    ...
    const dataCriacao       = get('DATA CRIAÇÃO DO AGENDAMENTO','DATA CRIACAO DO AGENDAMENTO','DATA CRIAÇÃO','DATA CRIACAO','DATA_CRIAÇÃO','DATA_CRIACAO');
    ...
    let mesLabel = '';
    if (dataAgendaParsed) mesLabel = MESES_PT[dataAgendaParsed.getMonth()] + '/' + dataAgendaParsed.getFullYear();

    return {
      _raw: row, unidadeExecutante: unidadeExec, unidadeSolicitante,
      cbo: cbof, especialidade: especialidade ? titleCase(especialidade.toString()) : '', nomeCBO,
      profissional, tipoAtendimento: tipoAtend, situacao, situacaoLabel: getSituacaoLabel(situacao),
      operadorCod: operCod, operador: getOperador(operCod),
      dataCriacao, dataCriacaoParsed, dataAgenda, dataAgendaParsed, mesAgendamento: mesLabel, distrito,
      valor: parseFloat((get('VALOR')||'0').replace(',','.')) || 0,
...
function populateMultiSelectOptions() {
  ...
  allData.forEach(r => {
    if (r.dataAgendaParsed && r.mesAgendamento) {
      ...
      mesesSet[d.getFullYear() * 100 + d.getMonth()] = r.mesAgendamento;
    }
  });
  ...
  Object.keys(multiSelectState).forEach(key => {
    ...
    const p = document.querySelector(`#${key} .multi-select-placeholder`);
    if (p) {
      if (key === 'multiSelectMes') p.setAttribute('data-default', 'Todos os meses');
      else if (key === 'multiSelectTipoServico') p.setAttribute('data-default', 'Todos');
      else if (key === 'multiSelectEspecialidade' || key === 'multiSelectUnidade') p.setAttribute('data-default', 'Todas');
      else p.setAttribute('data-default', 'Todos');
    }
  });
}
...
function applyFilters() {
  ...
  if (activeTab) renderChartsForTab(activeTab.getAttribute('data-tab'));
  else renderChartsForTab('visao-geral');
  if (document.getElementById('tab-tabela')?.classList.contains('active')) renderTable();
}
...
function updateKPIs() {
  ...
  animateCount('kpiAgendados', rec + fal);
...
function renderChartDistrito() {
  ...
  chartDistrito = new Chart(ctx, {
    type: 'bar',
    data: { labels, datasets: [{ label: 'Agendamentos', data, backgroundColor: labels.map(() => 'rgba(192,57,43,0.85)'), borderColor: labels.map(() => '#c0392b'), borderWidth: 2, borderRadius: 8, borderSkipped: false }] },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: { legend: { display: false }, tooltip: { ...TOOLTIP_BASE, callbacks: { label: ctx => ` ${fmt(ctx.raw)} agendamentos (${total > 0 ? (ctx.raw/total*100).toFixed(1) : 0}%)` } }, datalabels: { anchor: 'center', align: 'center', color: '#fff', font: { family: 'Inter', size: 13, weight: 'bold' }, formatter: val => val > 0 ? fmt(val) : '' } },
...
function renderChartTipoAtendimento() {
  ...
  chartTipoAtendimento = new Chart(ctx, {
    type: 'bar',
    data: { labels: ['1ª Consulta', 'Retorno'], datasets: [{ data: [pc, ret], backgroundColor: ['rgba(30,58,95,0.88)','rgba(39,174,96,0.88)'], borderColor: ['#1e3a5f','#27ae60'], borderWidth: 2, borderRadius: 8, borderSkipped: false }] },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: { legend: { display: false }, tooltip: TOOLTIP_BASE, datalabels: { anchor: 'center', align: 'center', color: '#fff', font: { family: 'Inter', size: 18, weight: 'bold' }, formatter: val => val > 0 ? fmt(val) : '' } },
      scales: { x: { ticks: { font: { family: 'Inter', size: 15, weight: '700' }, color: '#1e3a5f' }, grid: { display: false } }, y: { beginAtZero: true, ticks: { font: { family: 'Inter', size: 10 }, color: '#7a8fa6' }, grid: { display: false } } }
    }
  });
}
...
function renderChartEspecialidade() {
  ...
  chartEspecialidade = new Chart(ctx, {
    type: 'bar',
    data: { labels, datasets: [{ data, backgroundColor: labels.map((_,i) => PALETTE_PURPLE[i % PALETTE_PURPLE.length] + 'dd'), borderColor: labels.map((_,i) => PALETTE_PURPLE[i % PALETTE_PURPLE.length]), borderWidth: 2, borderRadius: 5, borderSkipped: false }] },
    options: {
...
function renderChartPrestador() {
  ...
  chartPrestador = new Chart(ctx, {
    type: 'bar',
    data: { labels, datasets: [{ data, backgroundColor: labels.map((_,i) => PALETTE_BLUE[i % PALETTE_BLUE.length] + 'dd'), borderColor: labels.map((_,i) => PALETTE_BLUE[i % PALETTE_BLUE.length]), borderWidth: 2, borderRadius: 6, borderSkipped: false }] },
    options: {
      responsive: true, maintainAspectRatio: false, indexAxis: 'y',
      plugins: { legend: { display: false }, tooltip: TOOLTIP_BASE, datalabels: { anchor: 'end', align: 'end', color: '#3d5166', font: { family: 'Inter', size: 13, weight: '800' }, formatter: val => fmt(val) } },
      layout: { padding: { right: 54 } },
...
function renderChartSituacao() {
  ...
  chartSituacao = new Chart(ctx, {
    type: 'bar',
    data: { labels, datasets: [{ data, backgroundColor: ['rgba(13,71,36,0.88)','rgba(26,122,63,0.88)','rgba(39,174,96,0.88)','rgba(52,152,85,0.88)','rgba(65,130,74,0.88)'], borderColor: ['#0d4724','#1a7a3f','#27ae60','#349855','#41824a'], borderWidth: 2, borderRadius: 8, borderSkipped: false }] },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: { legend: { display: false }, tooltip: { ...TOOLTIP_BASE, callbacks: { label: ctx => ` ${fmt(ctx.raw)} consultas (${total > 0 ? (ctx.raw/total*100).toFixed(1) : 0}%)` } }, datalabels: { anchor: 'center', align: 'center', color: '#fff', font: { family: 'Inter', size: 13, weight: 'bold' }, formatter: val => val > 0 ? fmt(val) : '' } },
      scales: { x: { ticks: { font: { family: 'Inter', size: 10, weight: '600' }, color: '#3d5166', maxRotation: 35 }, grid: { display: false } }, y: { beginAtZero: true, ticks: { font: { family: 'Inter', size: 10 }, color: '#7a8fa6' }, grid: { display: false } } }
    }
  });
}
...
function renderChartMeses() {
  ...
  chartMeses = new Chart(ctx, {
    type: 'doughnut',
    data: { labels, datasets: [{ data, backgroundColor: labels.map((_,i) => PALETTE_MONTHS[i % PALETTE_MONTHS.length]), borderColor: '#fff', borderWidth: 3 }] },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: {
        legend: { position: 'right', labels: { font: { family: 'Inter', size: 11, weight: '600' }, color: '#3d5166', padding: 14, generateLabels: (chart) => chart.data.labels.map((label, i) => ({ text: `${label}: ${fmt(chart.data.datasets[0].data[i])}`, fillStyle: chart.data.datasets[0].backgroundColor[i], hidden: false, index: i })) } },
        tooltip: { ...TOOLTIP_BASE, callbacks: { label: ctx => { const v = ctx.raw; return ` ${fmt(v)} agendamentos (${total > 0 ? (v/total*100).toFixed(1) : 0}%)`; } } },
...
function renderChartPrimeiraConsultaDistrito() {
  ...
  chartPrimeiraConsultaDistrito = new Chart(ctx, {
    type: 'bar',
    data: { labels: entries.map(e => e[0]), datasets: [{ data: entries.map(e => e[1]), backgroundColor: 'rgba(46,204,113,0.85)', borderColor: '#27ae60', borderWidth: 2, borderRadius: 8, borderSkipped: false }] },
    options: {
      responsive: true, maintainAspectRatio: false,
...
function renderChartRetornoDistrito() {
  ...
  chartRetornoDistrito = new Chart(ctx, {
    type: 'bar',
    data: { labels: entries.map(e => e[0]), datasets: [{ data: entries.map(e => e[1]), backgroundColor: 'rgba(155,89,182,0.85)', borderColor: '#8e44ad', borderWidth: 2, borderRadius: 8, borderSkipped: false }] },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: { legend: { display: false }, tooltip: TOOLTIP_BASE, datalabels: { anchor: 'center', align: 'center', color: '#fff', font: { family: 'Inter', size: 13, weight: 'bold' }, formatter: val => val > 0 ? fmt(val) : '' } },
      scales: { x: { ticks: { font: { family: 'Inter', size: 10, weight: '600' }, color: '#3d5166', maxRotation: 30 }, grid: { display: false } }, y: { beginAtZero: true, ticks: { font: { family: 'Inter', size: 10 }, color: '#7a8fa6' }, grid: { display: false } } }
...
function renderChartComparativoDistrito() {
  ...
  chartComparativoDistrito = new Chart(ctx, {
    ...
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: { legend: { position: 'top', labels: { font: { family: 'Inter', size: 12, weight: '600' }, color: '#3d5166', usePointStyle: true } }, tooltip: TOOLTIP_BASE, datalabels: { anchor: 'center', align: 'center', color: '#fff', font: { family: 'Inter', size: 11, weight: 'bold' }, formatter: val => val > 0 ? fmt(val) : '' } },
      scales: { x: { ticks: { font: { family: 'Inter', size: 10, weight: '600' }, color: '#3d5166', maxRotation: 35 }, grid: { display: false } }, y: { beginAtZero: true, ticks: { font: { family: 'Inter', size: 10 }, color: '#7a8fa6' }, grid: { display: false } } }
    }
  });
}
...
function renderChartDistritoRosca() {
  ...
  chartDistritoRosca = new Chart(ctx, {
    type: 'doughnut',
    data: { labels, datasets: [{ data, backgroundColor: ['#e74c3c','#3498db','#e67e22','#2ecc71','#9b59b6','#1abc9c','#f39c12','#e91e63'], borderColor: '#fff', borderWidth: 3 }] },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: { legend: { position: 'right', labels: { font: { family: 'Inter', size: 11, weight: '600' }, color: '#3d5166', padding: 14 } }, tooltip: { ...TOOLTIP_BASE, callbacks: { label: ctx => ` ${fmt(ctx.raw)} agendamentos (${total > 0 ? (ctx.raw/total*100).toFixed(1) : 0}%)` } }, datalabels: { color: '#fff', font: { family: 'Inter', size: 11, weight: 'bold' }, formatter: (val) => total > 0 ? (val/total*100).toFixed(0) + '%' : '' }, centerText: { enabled: true, value: fmt(total), label: 'Total', fontSize: 22, valueColor: '#1e3a5f', labelColor: '#7a8fa6' } }
    }
  });
}
...
function renderChartAbsenteismoEsp() {
  ...
  const entries = Object.entries(map).filter(([,v]) => (v.rec+v.fal+v.can) > 0).map(([k,v]) => ({ label: k, pct: parseFloat((v.fal/(v.rec+v.fal+v.can)*100).toFixed(1)), fal: v.fal, total: v.rec+v.fal+v.can })).sort((a,b) => b.pct - a.pct).slice(0, 15);
  ...
  chartAbsenteismoEsp = new Chart(ctx, {
    type: 'bar',
    data: { labels, datasets: [{ data, backgroundColor: bgs, borderColor: borders, borderWidth: 2, borderRadius: 6, borderSkipped: false }] },
    options: {
      responsive: true, maintainAspectRatio: false, indexAxis: 'y',
      plugins: { legend: { display: false }, tooltip: { ...TOOLTIP_BASE, callbacks: { label: (ctx) => { const e = entries[ctx.dataIndex]; return `${ctx.raw}% de absenteísmo (${fmt(e.fal)} faltosos de ${fmt(e.total)})`; } } }, datalabels: { anchor: 'end', align: 'end', color: '#3d5166', font: { family: 'Inter', size: 10, weight: 'bold' }, formatter: val => val + '%' } },
...
function renderChartAbsenteismoDist() {
  ...
  const entries = Object.entries(map).filter(([,v]) => (v.rec+v.fal+v.can) > 0).map(([k,v]) => ({ label: k, pct: parseFloat((v.fal/(v.rec+v.fal+v.can)*100).toFixed(1)), fal: v.fal, total: v.rec+v.fal+v.can })).sort((a,b) => b.pct - a.pct);
  const labels = entries.map(e => e.label);
  const data = entries.map(e => e.pct);
  const bgs = data.map(v => v >= 30 ? 'rgba(192,57,43,0.80)' : v >= 15 ? 'rgba(230,126,34,0.80)' : 'rgba(243,156,18,0.80)');
  const borders = data.map(v => v >= 30 ? '#c0392b' : v >= 15 ? '#e67e22' : '#f39c12');
  ...
  chartAbsenteismoDist = new Chart(ctx, {
    type: 'bar',
    data: { labels, datasets: [{ data, backgroundColor: bgs, borderColor: borders, borderWidth: 2, borderRadius: 7, borderSkipped: false }] },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: { legend: { display: false }, tooltip: { ...TOOLTIP_BASE, callbacks: { label: (ctx) => { const e = entries[ctx.dataIndex]; return `${ctx.raw}% de absenteísmo (${fmt(e.fal)} faltosos de ${fmt(e.total)})`; } } }, datalabels: { anchor: 'end', align: 'end', color: '#1a2a3a', font: { family: 'Inter', size: 11, weight: 'bold' }, formatter: val => val + '%' } },
...
function renderChartAbsenteismoMensal() {
  ...
  chartAbsenteismoMensal = new Chart(ctx, {
    type: 'line',
    data: { labels: sortedKeys, datasets: [{ data, borderColor: '#e74c3c', backgroundColor: 'rgba(231,76,60,0.1)', borderWidth: 3, pointRadius: 6, pointBackgroundColor: '#e74c3c', tension: 0.4, fill: true }] },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: { legend: { display: false }, tooltip: TOOLTIP_BASE, datalabels: { anchor: 'end', align: 'end', color: '#c0392b', font: { family: 'Inter', size: 11, weight: 'bold' }, formatter: val => val + '%' } },
      scales: { x: { ticks: { font: { family: 'Inter', size: 10, weight: '600' }, color: '#3d5166', maxRotation: 30 }, grid: { display: false } }, y: { beginAtZero: true, max: 100, ticks: { font: { family: 'Inter', size: 10 }, color: '#7a8fa6', callback: v => v + '%' }, grid: { display: false } } }
    }
  });
}
...
function renderChartAbsenteismoPrestador() {
  ...
  const entries = Object.entries(map).filter(([,v]) => (v.rec+v.fal+v.can) > 0).map(([k,v]) => ({ label: k, pct: parseFloat((v.fal/(v.rec+v.fal+v.can)*100).toFixed(1)) })).sort((a,b) => b.pct - a.pct).slice(0, 10);
  destroyChart(chartAbsenteismoPrestador);
  chartAbsenteismoPrestador = new Chart(ctx, {
    type: 'bar',
    data: { labels: entries.map(e => e.label), datasets: [{ data: entries.map(e => e.pct), backgroundColor: PALETTE_ORANGE.map(c => c + 'cc'), borderColor: PALETTE_ORANGE, borderWidth: 2, borderRadius: 6, borderSkipped: false }] },
    options: {
      responsive: true, maintainAspectRatio: false, indexAxis: 'y',
...
function renderChartCancelamentosDist() {
  ...
  const entries = Object.entries(map).filter(([,v]) => v.total > 0).map(([k,v]) => ({ label: k, pct: parseFloat((v.can/v.total*100).toFixed(1)) })).sort((a,b) => b.pct - a.pct);
  destroyChart(chartCancelamentosDist);
  chartCancelamentosDist = new Chart(ctx, {
    type: 'bar',
    data: { labels: entries.map(e => e.label), datasets: [{ data: entries.map(e => e.pct), backgroundColor: PALETTE_ROSE.map(c => c + 'bb'), borderColor: PALETTE_ROSE, borderWidth: 2, borderRadius: 7, borderSkipped: false }] },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: { legend: { display: false }, tooltip: TOOLTIP_BASE, datalabels: { anchor: 'end', align: 'end', color: '#1a2a3a', font: { family: 'Inter', size: 11, weight: 'bold' }, formatter: val => val + '%' } },
      layout: { padding: { top: 28 } },
      scales: { x: { ticks: { font: { family: 'Inter', size: 10, weight: '600' }, color: '#3d5166', maxRotation: 35 }, grid: { display: false } }, y: { beginAtZero: true, max: 100, ticks: { font: { family: 'Inter', size: 10 }, color: '#7a8fa6', callback: v => v + '%' }, grid: { display: false } } }
    }
  });
...
function renderChartCancelamentosEsp() {
  ...
  const entries = Object.entries(map).filter(([,v]) => v.total > 0).map(([k,v]) => ({ label: k, pct: parseFloat((v.can/v.total*100).toFixed(1)) })).sort((a,b) => b.pct - a.pct).slice(0, 15);
  destroyChart(chartCancelamentosEsp);
  chartCancelamentosEsp = new Chart(ctx, {
    type: 'bar',
    data: { labels: entries.map(e => e.label), datasets: [{ data: entries.map(e => e.pct), backgroundColor: PALETTE_GRAPE.map(c => c + 'bb'), borderColor: PALETTE_GRAPE, borderWidth: 2, borderRadius: 6, borderSkipped: false }] },
    options: {
      responsive: true, maintainAspectRatio: false, indexAxis: 'y',
      plugins: { legend: { display: false }, tooltip: TOOLTIP_BASE, datalabels: { anchor: 'end', align: 'end', color: '#3d5166', font: { family: 'Inter', size: 13, weight: '800' }, formatter: val => val + '%' } },
      layout: { padding: { right: 54 } },
      scales: { y: { ticks: { font: { family: 'Inter', size: 10 }, color: '#3d5166' }, grid: { display: false } }, x: { beginAtZero: true, max: 100, ticks: { font: { family: 'Inter', size: 10 }, color: '#7a8fa6', callback: v => v + '%' }, grid: { display: false } } }
    }
  });
...
function renderChartCancelamentosPrestador() {
  ...
  const entries = Object.entries(map).filter(([,v]) => v.total > 0).map(([k,v]) => ({ label: k, pct: parseFloat((v.can/v.total*100).toFixed(1)) })).sort((a,b) => b.pct - a.pct).slice(0, 10);
  destroyChart(chartCancelamentosPrestador);
  chartCancelamentosPrestador = new Chart(ctx, {
    type: 'bar',
    data: { labels: entries.map(e => e.label), datasets: [{ data: entries.map(e => e.pct), backgroundColor: PALETTE_ROSE.map(c => c + 'cc'), borderColor: PALETTE_ROSE, borderWidth: 2, borderRadius: 6, borderSkipped: false }] },
    options: {
      responsive: true, maintainAspectRatio: false, indexAxis: 'y',
      plugins: { legend: { display: false }, tooltip: TOOLTIP_BASE, datalabels: { anchor: 'end', align: 'end', color: '#3d5166', font: { family: 'Inter', size: 10, weight: 'bold' }, formatter: val => val + '%' } },
      layout: { padding: { right: 44 } },
...
function renderChartCancelamentosMensal() {
  ...
  const sortedKeys = Object.keys(map).sort();
  const data = sortedKeys.map(k => map[k].can);
  destroyChart(chartCancelamentosMensal);
  chartCancelamentosMensal = new Chart(ctx, {
    type: 'line',
    data: { labels: sortedKeys, datasets: [{ data, borderColor: '#c0392b', backgroundColor: 'rgba(192,57,43,0.1)', borderWidth: 3, pointRadius: 6, pointBackgroundColor: '#c0392b', tension: 0.4, fill: true }] },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: { legend: { display: false }, tooltip: TOOLTIP_BASE, datalabels: { anchor: 'end', align: 'end', color: '#c0392b', font: { family: 'Inter', size: 11, weight: 'bold' }, formatter: val => fmt(val) } },
      scales: { x: { ticks: { font: { family: 'Inter', size: 10, weight: '600' }, color: '#3d5166', maxRotation: 30 }, grid: { display: false } }, y: { beginAtZero: true, ticks: { font: { family: 'Inter', size: 10 }, color: '#7a8fa6' }, grid: { display: false } } }
    }
  });
}
...
function renderChartRecepcionadosDistrito() {
  ...
  chartRecepcionadosDistrito = new Chart(ctx, {
    type: 'bar',
    data: { labels: entries.map(e => e[0]), datasets: [{ data: entries.map(e => e[1]), backgroundColor: 'rgba(41,128,185,0.85)', borderColor: '#2980b9', borderWidth: 2, borderRadius: 8, borderSkipped: false }] },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: { legend: { display: false }, tooltip: TOOLTIP_BASE, datalabels: { anchor: 'center', align: 'center', color: '#fff', font: { family: 'Inter', size: 13, weight: 'bold' }, formatter: val => val > 0 ? fmt(val) : '' } },
      scales: { x: { ticks: { font: { family: 'Inter', size: 10, weight: '600' }, color: '#3d5166', maxRotation: 30 }, grid: { display: false } }, y: { beginAtZero: true, ticks: { font: { family: 'Inter', size: 10 }, color: '#7a8fa6' }, grid: { display: false } } }
...
function renderChartRecepcionadosEspecialidade() {
  ...
  chartRecepcionadosEspecialidade = new Chart(ctx, {
    type: 'bar',
    data: { labels: entries.map(e => e[0]), datasets: [{ data: entries.map(e => e[1]), backgroundColor: PALETTE_PURPLE.map(c => c + 'dd'), borderColor: PALETTE_PURPLE, borderWidth: 2, borderRadius: 5, borderSkipped: false }] },
    options: {
      responsive: true, maintainAspectRatio: false, indexAxis: 'y',
      plugins: { legend: { display: false }, tooltip: TOOLTIP_BASE, datalabels: { anchor: 'end', align: 'end', color: '#3d5166', font: { family: 'Inter', size: 13, weight: '800' }, formatter: val => fmt(val) } },
      layout: { padding: { right: 54 } },
...
function renderChartRecepcionadosPrestador() {
  ...
  chartRecepcionadosPrestador = new Chart(ctx, {
    type: 'bar',
    data: { labels: entries.map(e => e[0]), datasets: [{ data: entries.map(e => e[1]), backgroundColor: PALETTE_BLUE.map(c => c + 'dd'), borderColor: PALETTE_BLUE, borderWidth: 2, borderRadius: 6, borderSkipped: false }] },
    options: {
      responsive: true, maintainAspectRatio: false, indexAxis: 'y',
      plugins: { legend: { display: false }, tooltip: TOOLTIP_BASE, datalabels: { anchor: 'end', align: 'end', color: '#3d5166', font: { family: 'Inter', size: 13, weight: '800' }, formatter: val => fmt(val) } },
      layout: { padding: { right: 54 } },
...
function renderChartRecepcionadosMensal() {
  ...
  chartRecepcionadosMensal = new Chart(ctx, {
    type: 'line',
    data: { labels: sortedKeys, datasets: [{ data, borderColor: '#27ae60', backgroundColor: 'rgba(39,174,96,0.1)', borderWidth: 3, pointRadius: 6, pointBackgroundColor: '#27ae60', tension: 0.4, fill: true }] },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: { legend: { display: false }, tooltip: TOOLTIP_BASE, datalabels: { anchor: 'end', align: 'end', color: '#27ae60', font: { family: 'Inter', size: 11, weight: 'bold' }, formatter: val => fmt(val) } },
      scales: { x: { ticks: { font: { family: 'Inter', size: 10, weight: '600' }, color: '#3d5166', maxRotation: 30 }, grid: { display: false } }, y: { beginAtZero: true, ticks: { font: { family: 'Inter', size: 10 }, color: '#7a8fa6' }, grid: { display: false } } }
    }
  });
}
...
function renderChartTransferidosEspecialidade() {
  ...
  chartTransferidosEspecialidade = new Chart(ctx, {
    type: 'bar',
    data: { labels: entries.map(e => e[0]), datasets: [{ data: entries.map(e => e[1]), backgroundColor: PALETTE_GRAPE.map(c => c + 'dd'), borderColor: PALETTE_GRAPE, borderWidth: 2, borderRadius: 5, borderSkipped: false }] },
    options: {
      responsive: true, maintainAspectRatio: false, indexAxis: 'y',
      plugins: { legend: { display: false }, tooltip: TOOLTIP_BASE, datalabels: { anchor: 'end', align: 'end', color: '#3d5166', font: { family: 'Inter', size: 13, weight: '800' }, formatter: val => fmt(val) } },
      layout: { padding: { right: 54 } },
...
function renderChartTransferidosPrestador() {
  ...
  chartTransferidosPrestador = new Chart(ctx, {
    type: 'bar',
    data: { labels: entries.map(e => e[0]), datasets: [{ data: entries.map(e => e[1]), backgroundColor: PALETTE_TEAL.map(c => c + 'dd'), borderColor: PALETTE_TEAL, borderWidth: 2, borderRadius: 6, borderSkipped: false }] },
    options: {
      responsive: true, maintainAspectRatio: false, indexAxis: 'y',
      plugins: { legend: { display: false }, tooltip: TOOLTIP_BASE, datalabels: { anchor: 'end', align: 'end', color: '#3d5166', font: { family: 'Inter', size: 13, weight: '800' }, formatter: val => fmt(val) } },
      layout: { padding: { right: 54 } },
...
function renderChartTransferidosMensal() {
  ...
  chartTransferidosMensal = new Chart(ctx, {
    type: 'line',
    data: { labels: sortedKeys, datasets: [{ data, borderColor: '#7d3c98', backgroundColor: 'rgba(125,60,152,0.1)', borderWidth: 3, pointRadius: 6, pointBackgroundColor: '#7d3c98', tension: 0.4, fill: true }] },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: { legend: { display: false }, tooltip: TOOLTIP_BASE, datalabels: { anchor: 'end', align: 'end', color: '#7d3c98', font: { family: 'Inter', size: 11, weight: 'bold' }, formatter: val => fmt(val) } },
      scales: { x: { ticks: { font: { family: 'Inter', size: 10, weight: '600' }, color: '#3d5166', maxRotation: 30 }, grid: { display: false } }, y: { beginAtZero: true, ticks: { font: { family: 'Inter', size: 10 }, color: '#7a8fa6' }, grid: { display: false } } }
    }
  });
}
...
function buildTableData() {
  ...
  filteredData.forEach(r => {
    ...
    if (!map[key]) map[key] = { distrito: r.distrito, unidadeSolicitante: r.unidadeSolicitante, tipoServico: r.tipoAtendimento, cbo: r.cbo, profissional: r.profissional, unidadeExecutante: r.unidadeExecutante, age: 0, rec: 0, fal: 0, can: 0, tra: 0 };
  ...
  tableData = Object.values(map).map(r => {
    ...
    return { ...r, totalAgendamentos, pctAbsenteismo: totalAgendamentos > 0 ? parseFloat((r.fal/totalAgendamentos*100).toFixed(1)) : 0 };
...
function filterTable() {
  ...
  tableSearched = !q ? [...tableData] : tableData.filter(r => (r.distrito||'').toLowerCase().includes(q) || (r.unidadeSolicitante||'').toLowerCase().includes(q) || (r.tipoServico||'').toLowerCase().includes(q) || (r.cbo||'').toLowerCase().includes(q) || (r.profissional||'').toLowerCase().includes(q) || (r.unidadeExecutante||'').toLowerCase().includes(q));
  currentPage = 1;
  renderTable();
}
...
function sortTable(col) {
  ...
  tableSearched.sort((a,b) => { const va = a[keys[col]] ?? '', vb = b[keys[col]] ?? ''; return (sortAscFlag ? 1 : -1) * (typeof va === 'number' ? va - vb : va.toString().localeCompare(vb.toString(), 'pt-BR')); });
  renderTable();
}
...
function renderTable() {
  ...
  if (slice.length === 0) {
    ...
    `).join('');
    const sAge = tableSearched.reduce((s,r) => s + r.age, 0), sRec = tableSearched.reduce((s,r) => s + r.rec, 0), sFal = tableSearched.reduce((s,r) => s + r.fal, 0), sCan = tableSearched.reduce((s,r) => s + r.can, 0), sTra = tableSearched.reduce((s,r) => s + r.tra, 0), sTotal = tableSearched.reduce((s,r) => s + r.totalAgendamentos, 0);
    tfoot.innerHTML = `<tr><td colspan="6"><i class="fas fa-calculator" style="margin-right:6px;"></i>TOTAL GERAL (${fmt(tableSearched.length)} linhas)</td><td class="text-center">${fmt(sAge)}</td><td class="text-center">${fmt(sRec)}</td><td class="text-center">${fmt(sFal)}</td><td class="text-center">${fmt(sCan)}</td><td class="text-center">${fmt(sTra)}</td><td class="text-center">${fmt(sTotal)}</td><td class="text-center">${sTotal > 0 ? (sFal/sTotal*100).toFixed(1) : 0}%</td></tr>`;
  }
...
function renderPagination(cur, total) {
  ...
  else { pages = [1]; if (cur > 3) pages.push('...'); for (let i=Math.max(2,cur-1); i<=Math.min(total-1,cur+1); i++) pages.push(i); if (cur < total-2) pages.push('...'); pages.push(total); }
  pages.forEach(p => { if (p === '...') html += `<button class="page-btn" disabled>…</button>`; else html += `<button class="page-btn ${p===cur?'active':''}" onclick="goPage(${p})">${p}</button>`; });
...
function exportExcel() {
  ...
  setTimeout(() => {
    try {
      const wsData = filteredData.map(r => ({ 'Unidade Executante': r.unidadeExecutante, 'Unidade Solicitante': r.unidadeSolicitante, 'Distrito': r.distrito, 'Especialidade (CBO)': r.cbo, 'Profissional': r.profissional, 'Tipo Atendimento': r.tipoAtendimento, 'Situação': r.situacaoLabel, 'Operador': r.operador, 'Data Agenda': r.dataAgenda, 'Data Criação': r.dataCriacao, 'Mês Agendamento': r.mesAgendamento }));
      const wsSummary = tableData.map(r => ({ 'Distrito': r.distrito, 'Unidade Solicitante': r.unidadeSolicitante, 'Tipo de Serviço': r.tipoServico, 'CBO / Especialidade': r.cbo, 'Profissional Médico': r.profissional, 'Unidade Executante': r.unidadeExecutante, 'AGE': r.age, 'REC': r.rec, 'FAL': r.fal, 'CAN': r.can, 'TRA': r.tra, 'Total': r.totalAgendamentos, '% Absenteísmo': r.pctAbsenteismo + '%' }));
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(wsData), 'Dados');
      XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(wsSummary), 'Consolidado');
      XLSX.writeFile(wb, `CMC_Consolidado_${new Date().toISOString().slice(0,10).replace(/-/g,'')}.xlsx`);
    } catch(e) { alert('Erro ao gerar Excel.'); }
    btn.disabled = false; btn.innerHTML = '<i class="fas fa-file-excel"></i> Excel';
  }, 100);
}
...
function showError(msg) { setStatus('Erro', false); showLoading(false); alert(msg); }
...
function initDatePickers() {
  window._fpInicio = flatpickr('#filterDataInicio', {
    locale: 'pt', dateFormat: 'd/m/Y', allowInput: false, disableMobile: false,
