...
const URL_FILA_RETROATIVA = "https://docs.google.com/spreadsheets/d/1ax8ZpVRSZnDkTm_T1GY37ybSgrlP_8Rk/export?format=csv&gid=364381141";
...
async function loadAllData() {
  ...
  try {
    ...
    const [filaRaw, agVivverRaw, financeiroRaw, agendadosRaw, filaRetroativaRaw] = await Promise.all([
    ...
    // Processa Fila Retroativa
    let latestRetroativaDateValue = "";
    let latestRetroativaDateSortValue = -1;
    ...
    dadosFilaRetroativa = filaRetroativaRaw.map(r => {
      ...
      const grupoCodigo = extractGrupoCodigo(grupoRaw);
      const complexidade = normalizeText(getField(r, ["Complexidade- Sigtap"]));
      ...
      if (dataCorteNorm) {
        const sortVal = periodoSortValue(dataCorteNorm);
        if (sortVal > latestRetroativaDateSortValue) {
          latestRetroativaDateSortValue = sortVal;
          latestRetroativaDateValue = dataCorteNorm;
        }
      }
      return {
    ...
    // Atualizar o elemento da data de corte retroativa se existir
    const dataCorteRetroativaElement = el("kFilaRetroativaDataCorte");
    if (dataCorteRetroativaElement) {
      dataCorteRetroativaElement.innerHTML = `<i class="fa-regular fa-calendar"></i> Data de corte: ${latestDataCorteRetroativa}`;
    }
    ...
    allPeriodos = sortPeriodos([
      ...
      ...dadosFilaRetroativa.map(d => d.dataCorte)
    ]);

    console.log("Períodos encontrados:", allPeriodos);
    console.log("Dados da Fila Retroativa carregados:", dadosFilaRetroativa.length);
    console.log("Datas da Fila Retroativa:", dadosFilaRetroativa.map(d => d.dataCorte));
...
function updateLastUpdate() {
  ...
  if (last) {
    last.textContent = `Última atualização: ${now.toLocaleDateString("pt-BR")} ${now.toLocaleTimeString("pt-BR")}`;
  }
}
...
function applyFilters() {
  ...
  // Filtrar também os dados retroativos
  ...
  const kAgendadosTotal = el("kAgendadosTotal");
  const kFaturadosQtd = el("kFaturadosQtd");
  const kFinanceiro = el("kFinanceiro");

  if (kFila) kFila.innerText = totalFila.toLocaleString("pt-BR");
  if (kRecepcionados) kRecepcionados.innerText = totalRecepcionados.toLocaleString("pt-BR");
  if (kFaltosos) kFaltosos.innerText = totalFaltosos.toLocaleString("pt-BR");
  if (kAgendadosTotal) kAgendadosTotal.innerText = totalAgendados.toLocaleString("pt-BR");
  if (kFaturadosQtd) kFaturadosQtd.innerText = totalFaturadosQtd.toLocaleString("pt-BR");
  if (kFinanceiro) kFinanceiro.innerText = formatMoney(totalFinanceiro);
  ...
  if (taxaFaturamento >= 80) {
    fatColor = "#059669";
    fatBadgeBg = "rgba(5,150,105,.20)";
    fatBadgeColor = "#065f46";
    fatBadgeText = "✓ Ótimo";
  } else if (taxaFaturamento >= 50) {
    fatColor = "#d97706";
    fatBadgeBg = "rgba(217,119,6,.18)";
    fatBadgeColor = "#92400e";
    fatBadgeText = "⚠ Regular";
  } else {
    fatColor = "#dc2626";
    fatBadgeBg = "rgba(220,38,38,.16)";
    fatBadgeColor = "#991b1b";
    fatBadgeText = "✗ Crítico";
  }
  ...
  const gFatAg = el("gFatAg");
  const gFatFat = el("gFatFat");
  const gFatDiff = el("gFatDiff");
  
  if (gFatAg) gFatAg.textContent = totalAgendados.toLocaleString("pt-BR");
  ...
  if (taxaAbsenteismo <= 10) {
    absColor = "#059669";
    absBadgeBg = "rgba(5,150,105,.20)";
    absBadgeColor = "#065f46";
    absBadgeText = "✓ Excelente";
  } else if (taxaAbsenteismo <= 20) {
    absColor = "#3b82f6";
    absBadgeBg = "rgba(59,130,246,.18)";
    absBadgeColor = "#1d4ed8";
    absBadgeText = "✓ Bom";
  } else if (taxaAbsenteismo <= 35) {
    absColor = "#d97706";
    absBadgeBg = "rgba(217,119,6,.18)";
    absBadgeColor = "#92400e";
    absBadgeText = "⚠ Atenção";
  } else {
    absColor = "#dc2626";
    absBadgeBg = "rgba(220,38,38,.16)";
    absBadgeColor = "#991b1b";
    absBadgeText = "✗ Crítico";
  }
  ...
  const gAbsAg = el("gAbsAg");
  const gAbsRec = el("gAbsRec");
  const gAbsAus = el("gAbsAus");
  
  if (gAbsAg) gAbsAg.textContent = agVivverTotal.toLocaleString("pt-BR");
  ...
  // Passar os dados retroativos filtrados para a visão geral
  renderVisaoGeral(filteredFila, filteredAgVivver, filteredAgendados, filteredFinanceiro, filteredFilaRetroativa);
...
function renderMixedEvolutionChart(periods, filaPorMes, ofertaPorMes, recepcionadosPorMes, faltososPorMes) {
  ...
  console.log("===== GRÁFICO DE EVOLUÇÃO =====");
  console.log("Períodos:", periods);
  console.log("Dados de Fila (com retroativos):", filaPorMes);
  ...
  console.log("Valores da Fila no gráfico:", filaData);
  ...
  charts.cEvolucao = new Chart(canvas.getContext("2d"), {
    data: {
      ...
      datasets: [
        ...
        {
          type: "line",
          label: "Fila de Espera (Atual + Retroativa)",
          data: filaData,
          borderColor: "#dc2626",
...
// FUNÇÃO CORRIGIDA - Agora inclui os dados retroativos
function renderVisaoGeral(filteredFila, filteredAgVivver, filteredAgendados, filteredFinanceiro, filteredFilaRetroativa = []) {
  // COMBINAR os dados da fila atual com os dados retroativos
  const todasFila = [...filteredFila, ...filteredFilaRetroativa];
  
  console.log("===== RENDER VISAO GERAL =====");
  console.log("Fila atual (filtrada):", filteredFila.length);
  console.log("Fila retroativa (filtrada):", filteredFilaRetroativa.length);
  console.log("Total combinado:", todasFila.length);
  console.log("Dados retroativos por data:", filteredFilaRetroativa.map(d => ({ data: d.dataCorte, valor: d.fila })));
  
  // Obter todos os períodos a partir dos dados COMBINADOS
  const periods = getPeriodsFromFilteredData(todasFila, filteredAgVivver, filteredAgendados, filteredFinanceiro);
  
  // Agregar a fila usando os dados COMBINADOS
  ...
  // Renderizar gráfico com os dados COMBINADOS
...
function renderTableBodyFisico() {
  ...
  tbody.innerHTML = rows.map(r => `
    <tr>
      ...
      <td class="text-right">${formatMoney(r.financeiroValor)}
  tbody.innerHTML = rows.map(r => `
    <tr>
      <td title="${escapeHtml(r.estabelecimento)}">${escapeHtml(truncateLabel(r.estabelecimento, 60))}</td>
      <td>${escapeHtml(r.mes || "-")}</td>
...
function renderFilaRetroativa() {
  ...
  const kFilaRetroativaTotal = el("kFilaRetroativaTotal");
  const kFilaRetroativaMedia = el("kFilaRetroativaMedia");
  const kFilaRetroativaDataCorte = el("kFilaRetroativaDataCorte");
  
  if (kFilaRetroativaTotal) kFilaRetroativaTotal.innerText = totalFilaRetroativa.toLocaleString("pt-BR");
  if (kFilaRetroativaMedia) {
    const media = dadosFilaRetroativa.length > 0 ? totalFilaRetroativa / dadosFilaRetroativa.length : 0;
    kFilaRetroativaMedia.innerText = media.toFixed(1);
  }
  if (kFilaRetroativaDataCorte) {
    kFilaRetroativaDataCorte.innerHTML = `<i class="fa-regular fa-calendar"></i> Data de corte: ${latestDataCorteRetroativa}`;
  }
  ...
  renderPercentageTotalTable("tableRetroativaEspecialidadeBody", filaRetroativaEspecialidade, "#8b5cf6");
  renderPercentageTotalTable("tableRetroativaProcedimentoBody", filaRetroativaProcedimento, "#a855f7");
  ...
  renderTabelaRetroativaDetalhada(dadosFilaRetroativa);
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (btnClear) {
    btnClear.addEventListener("click", () => {
      ...
      const tableMonthFilterFisico = el("tableMonthFilterFisico");
      if (tableMonthFilterFisico) tableMonthFilterFisico.value = "";
      const tabelaMonthFilterEspec = el("tabelaMonthFilterEspec");
      if (tabelaMonthFilterEspec) tabelaMonthFilterEspec.value = "";
      const msSearchSub = el("msSearchSub");
      if (msSearchSub) msSearchSub.value = "";
      const msSearchEsp = el("msSearchEsp");
      if (msSearchEsp) msSearchEsp.value = "";
